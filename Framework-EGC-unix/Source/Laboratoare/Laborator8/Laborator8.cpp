...
void Laborator8::Init()
{
	...
	// Create a shader program for drawing face polygon with the color of the normal
	{
		...
		shader->AddShader("Resources/Shaders/VertexShader8.glsl", GL_VERTEX_SHADER);
		shader->AddShader("Resources/Shaders/FragmentShader8.glsl", GL_FRAGMENT_SHADER);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		{
			cutOffAngle += deltaTime * ANG_SPEED;
			if (cutOffAngle > 360.f) {
				cutOffAngle = 360.f;
			}
			cout << "CEVA " << cutOffAngle;
		}
	...
	if (window->KeyHold(GLFW_KEY_UP)) {
		lightDirection = glm::vec3(glm::rotate(glm::mat4(1), deltaTime * 2.f, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1));
	}
	if (window->KeyHold(GLFW_KEY_DOWN)) {
		lightDirection = glm::vec3(glm::rotate(glm::mat4(1), -deltaTime * 2.f, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1));
	}
	if (window->KeyHold(GLFW_KEY_LEFT)) {
		lightDirection = glm::vec3(glm::rotate(glm::mat4(1), deltaTime * 2.f, glm::vec3(0, 1, 0)) * glm::vec4(lightDirection, 1));
	}
